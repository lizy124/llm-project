...
SERVED_MODEL_NAME="${SERVED_MODEL_NAME:-qwen3-32b-layerwise}"
...
: > "$TEST_LOG"
echo "RUNNING: Qwen3-32B PDMix layerwise (memcache) KV Pool validation" | tee "$STATUS_FILE"
...
if ! grep -Eiq 'AscendStoreConnector|Initializing Memcache store|KV pool|kv pool|start_load_kv|store KV cache|lookup_rpc_port|layerwise|use_layerwise' "$LOG_FILE"; then
  fail_failed "server log does not show AscendStore/Memcache KV Pool activity"
fi

pass_msg "Qwen3-32B PDMix layerwise (memcache) KV Pool smoke and repeated-prefix validation passed"
