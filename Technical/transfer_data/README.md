...
> 边界说明：`refactor/layerwise/` 承载 layerwise **落地重构的完整记录**（审核、开发、E2E 证据）；项目初期的**提案/规划**（需求分析、设计、计划和测试方案）已归档在 `Proposals/archive/Layerwise-Pooling-Optimization/`。二者一个是"方案"，一个是"落地记录"，互斥存放、不相往来。
