...
| C3 | 改动后是否引入回归 | **无回归，80 passed** | §5.1；[logs/ut_after_fix.log](test_record/logs/ut_after_fix.log) |
| C4 | 新增测试是否有效（能抓住缺陷） | **有效，基线 4 failed** | §5.2；[logs/ut_before_fix_baseline.log](test_record/logs/ut_before_fix_baseline.log) |
| C5 | 冗余消除是否达成（issue 验收标准 2） | **达成**：每块 2.25 次 → 1 次 | §5.3 |
| C6 | 边界场景（含新增 3 类）是否全部正确 | **全部正确，用例全覆盖** | §5.4 |
...
| `group_block_hashes` 为空 | `..._empty_hashes` | ✅ |
...
[logs/ut_after_fix.log](test_record/logs/ut_after_fix.log) 尾部：

```
================= 80 passed, 14 warnings in 1.66s ================
```

覆盖 6 个既有测试类 + 8 个新增用例，**无失败、无报错**（14 条 warnings 均为上游 torch `jit.script_method` 弃用告警，与本次改动无关）。
...
- 唯一 passed 项为 `..._empty_hashes`：空 hash 列表场景改动前后转换次数均为 0，符合预期。
...
- 抬升越界 / 空 hash：转换计数为 0，`batch_alloc` 不被调用 —— 与预期一致。
...
4. **测试有效**：改动前基线下新计数测试 **4 failed / 1 passed**，证明测试精确刻画"每 hash 1 次"约束。
5. **冗余消除达成**：`block_hash_to_str` 每块从 **~2.25 → 1 次（-56%）**，前方仍存临时对象减少，满足 issue #14148 验收标准 2。
6. **边界完备**：含复核新增的 3 类边界（严格大于抬升、空 hash、重复 hash）全部经测试验证。
7. **语义澄清**：明确跳过块 GVA 在传输数组为 0 的既有语义，并写入文档供维护。

**总体：本次修复实现正确、测试充分、目标达成，可进入冒烟与提交流程。**