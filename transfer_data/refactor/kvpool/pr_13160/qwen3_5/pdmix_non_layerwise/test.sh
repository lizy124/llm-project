...
: > "$TEST_LOG"
echo "RUNNING: Qwen3.5-397B-A17B PDMix non-layerwise KV Pool validation" | tee "$STATUS_FILE"
...
smoke_payload = {
    "model": model,
    "messages": [
        {"role": "user", "content": "Give one short sentence about KV cache pooling."}
    ],
    "max_tokens": 32,
    "temperature": 0.0,
}
...
shared_prefix = (
    "You are validating KV cache pool reuse for Qwen3.5. "
...
if grep -Eiq 'Traceback \(most recent call last\)|RuntimeError:|Segmentation fault|core dumped|ERR[0-9]+|HCCL.*(error|failed)|NPU.*(error|failed)|OutOfMemoryError|out of memory|OOM|Memory_Allocation_Failure|ACL.*(memory|alloc)|Store initialization failed|Configuration loading failed' "$LOG_FILE"; then
  grep -Ein 'Traceback \(most recent call last\)|RuntimeError:|Segmentation fault|core dumped|ERR[0-9]+|HCCL.*(error|failed)|NPU.*(error|failed)|OutOfMemoryError|out of memory|OOM|Memory_Allocation_Failure|ACL.*(memory|alloc)|Store initialization failed|Configuration loading failed' "$LOG_FILE" | tail -80 | tee -a "$TEST_LOG" >&2
  fail_failed "server log contains fatal error patterns"
fi

if ! grep -Eiq 'AscendStoreConnector|Initializing Memcache store|Mooncake|mooncake|KV pool|kv pool|start_load_kv|store KV cache|lookup_rpc_port' "$LOG_FILE"; then
  fail_failed "server log does not show AscendStore/Mooncake KV Pool activity"
fi

pass_msg "Qwen3.5-397B-A17B PDMix non-layerwise KV Pool smoke and repeated-prefix validation passed"
