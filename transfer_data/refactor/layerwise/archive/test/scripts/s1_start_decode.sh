#!/usr/bin/env bash
# s1_start_decode.sh — 场景1 D 侧:DSV2-Lite TP=4,MooncakeLayerwiseConnector kv_consumer
set -Eeuo pipefail
...
LOG_FILE="$RESULT_DIR/decode.log"
PID_FILE="$RESULT_DIR/decode.pid"
STATUS_FILE="$RESULT_DIR/decode_status.txt"
...
{
  echo "date=$(date -Is)"
  echo "scenario=s1_pd_multiconn role=decode model=$MODEL_PATH tp=$TENSOR_PARALLEL_SIZE chips=$ASCEND_RT_VISIBLE_DEVICES"
  echo "kv_config=$KV_CONFIG"
} > "$RESULT_DIR/decode_env.txt" 2>&1

rm -f "$LOG_FILE" "$PID_FILE"
echo "RUNNING: starting decode" | tee "$STATUS_FILE"
...
while (( SECONDS < DEADLINE )); do
  if ! kill -0 "$(cat "$PID_FILE")" 2>/dev/null; then
    ...
    fail "decode exited before readiness"
  fi
  if curl -s --max-time 5 "http://127.0.0.1:$SERVER_PORT/v1/models" | grep -q '"data"'; then
    echo "PASS: decode ready at http://127.0.0.1:$SERVER_PORT" | tee -a "$STATUS_FILE"
    exit 0
  fi
  sleep 10
done
tail -100 "$LOG_FILE" >&2 || true
fail "decode not ready within ${READINESS_TIMEOUT_S}s"
