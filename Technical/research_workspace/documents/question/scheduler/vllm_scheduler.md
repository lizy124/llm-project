...
如果是 EAGLE 家族，`num_prefill_lookahead` 还会根据是否启用
multi-module MTP 设置为 `num_spec_tokens` 或 1，用于 prefill chunk 边界、
encoder scheduling 偏移和 cache 释放。配置了
`num_speculative_tokens_per_batch_size` 时，`dynamic_sd_lookup` 会按 batch
size 选择下一轮的 spec token 数。
...
schedule()
  │
  ├─ 初始化本轮预算 token_budget / input_budget / draft_slots
  ├─ 调度 running 请求
  │    ├─ 计算本轮要处理多少 token
  │    ├─ 处理 encoder input
  │    ├─ 分配 KV block
...
所以本轮能调度多少 token，首先受 `max_num_scheduled_tokens` 限制；同时
模型输入总量还受 `input_budget` 限制，每个已调度请求会扣除
`num_new_tokens + draft_slots`。单个请求的计算 token 上限至少要取：
...
3. `input_budget - draft_slots` 为 speculative drafter 预留输入位置；
4. `max_model_len` 限制上下文长度。
...
blocked 状态恢复后，Scheduler 还会检查 stale output：
...
这是为了等待仍可交付的旧输出帧排空，避免请求恢复后对同一位置重新采样。
...
- `shared_prefix_boundary`：稀疏 KV group 需要保留的共享前缀边界；普通模型通常为 0。
...
如果没有启用 chunked prefill，并且当前 prompt 剩余 token 超过本轮
...
同时，对于 async scheduling 中仍在途的 output，preempt 会记录
`num_stale_output_tokens` 并清零 `num_output_placeholders`。`drop_stale_output`
决定 stale frame 是直接丢弃还是仍然交付；交付模式不会用 stale frame 做
spec reject 回退。
...
- `num_stale_output_tokens`：reset / force-preempt 后仍在途的 stale output token 数；
- `drop_stale_output`：是否丢弃 stale output，而不是交付给客户端；
